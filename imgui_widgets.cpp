...
extern ImFont* Alexandria_Medium;
...
extern ImFont* Alexandria_Semmi_4;
bool ImGui::SelectButtons(const char* label, const char* label_1, ImColor text_color, const char* percent, const char* message, float percent_circle, const ImVec2& size_arg, ImGuiButtonFlags flags)
{
    ImGuiWindow* window = GetCurrentWindow();
    if (window->SkipItems)
        return false;
    ...
    bool topHalf = true;
    ImVec2 center(bb.Min.x, bb.Min.y);
    float radius = 30.f;

    percent_circle = ImClamp(percent_circle, 0.0f, 1.0f);

    const int numSegments = 360;
    const float angleStep = IM_PI * 2.f / numSegments;

    // Set the start angle to the left side
    float startAngle = IM_PI;
    ...
    draw->PathStroke(ImColor(text_color), false, 12.0f);

    draw->AddCircle(ImVec2(bb.Min.x, bb.Min.y), 30.f, GetColorU32(c::circle_in), 0, 12.0f);

    PushFont(Alexandria_Semmi_4);
    GetWindowDrawList()->AddText(ImVec2(bb.Min.x + 55, bb.Min.y - 37), ImColor(text_color), percent);
    GetWindowDrawList()->AddText(0, 20.f, ImVec2(bb.Min.x + 55, bb.Min.y-7), ImColor(255, 255, 255, 255), label_1);
    GetWindowDrawList()->AddText(0, 14.f, ImVec2(bb.Min.x + 55, bb.Min.y + 15), ImColor(55, 55, 55, 255), message);
...
bool ImGui::ButtonEx( const char* label, const ImVec2& size_arg, ImGuiButtonFlags flags)
{
    ...
    it_anim->second.background = ImLerp(it_anim->second.background, IsItemActive() || hovered ? c::main_color : c::main_color, g.IO.DeltaTime * 8.f);
    it_anim->second.shadow = ImLerp(it_anim->second.shadow, IsItemActive() || hovered ? c::main_color : c::rect_input_in, g.IO.DeltaTime * 8.f);

    it_anim->second.text = ImLerp(it_anim->second.text, IsItemActive() || hovered ? c::text_active : c::text_active, g.IO.DeltaTime * 8.f);

    it_anim->second.size = ImLerp(it_anim->second.size, IsItemActive() || hovered ? 87.f : 0.f, g.IO.DeltaTime * 8.f);

    const ImU32 col = GetColorU32((held && hovered) ? ImGuiCol_ButtonActive : hovered ? ImGuiCol_ButtonHovered : ImGuiCol_Button);
    RenderNavHighlight(bb, id);
    //RenderFrame(bb.Min, bb.Max, GetColorU32(it_anim->second.background), true, 6);
    GetWindowDrawList()->AddShadowRect(bb.Min, bb.Max, GetColorU32(it_anim->second.shadow), it_anim->second.size,ImVec2(0,0), ImDrawFlags_ShadowCutOutShapeBackground, 60.f);
    GetWindowDrawList()->AddRectFilled(bb.Min, bb.Max, GetColorU32(it_anim->second.background), 17.f);

    if (g.LogEnabled)
        LogSetNextTextDecoration("[", "]");
    PushFont(Alexandria_Medium);
...
extern ImFont* Alexandria_Regular_3;
bool ImGui::Button_Back(const char* label, const char* text, ImTextureID image, const ImVec2& size_arg, ImGuiButtonFlags flags)
{
    ImGuiWindow* window = GetCurrentWindow();
    if (window->SkipItems)
        return false;
    ...
    it_anim->second.background = ImLerp(it_anim->second.background, IsItemActive() || hovered ? ImColor(21,21,21,255) : ImColor(21, 21, 21, 255), g.IO.DeltaTime * 8.f);
    it_anim->second.shadow = ImLerp(it_anim->second.shadow, IsItemActive() || hovered ? c::main_color : c::rect_input_in, g.IO.DeltaTime * 8.f);

    it_anim->second.text = ImLerp(it_anim->second.text, IsItemActive() || hovered ? c::text_active : c::text_back, g.IO.DeltaTime * 8.f);
    ...
    GetWindowDrawList()->AddRectFilled(bb.Min, bb.Max, GetColorU32(it_anim->second.background), 5.f);

    GetWindowDrawList()->AddImage(image, bb.Min + ImVec2(6, 5), bb.Min + ImVec2(19, 20), ImVec2(0, 0), ImVec2(1, 1), GetColorU32(it_anim->second.text));

    if (g.LogEnabled)
        LogSetNextTextDecoration("[", "]");
    PushFont(Alexandria_Regular_3);

    PushStyleColor(ImGuiCol_Text, GetColorU32(it_anim->second.text));
    RenderTextClipped(bb.Min + ImVec2(18,3) + style.FramePadding, bb.Max - style.FramePadding, label, NULL, &label_size, style.ButtonTextAlign, &bb);
    PopStyleColor();
    PopFont();

    //PushFont(Alexandria_Semmi_4);

    GetWindowDrawList()->AddText(ImVec2(bb.Min.x - 204, bb.Min.y + 1), GetColorU32(it_anim->second.text), text);
...
bool ImGui::Help_Button(const char* label, ImTextureID image, const ImVec2& size_arg, ImGuiButtonFlags flags)
{
    ImGuiWindow* window = GetCurrentWindow();
    if (window->SkipItems)
        return false;
    ...
    it_anim->second.background = ImLerp(it_anim->second.background, IsItemActive() || hovered ? c::main_color : c::main_color_1, g.IO.DeltaTime * 12.f);

    //it_anim->second.background1 = ImLerp(it_anim->second.background1, IsItemActive() || hovered ? c::button_multi_2 : c::rect_input, g.IO.DeltaTime * 12.f);

    it_anim->second.text = ImLerp(it_anim->second.text, IsItemActive() || hovered ? ImColor(255, 255, 255, 255) : ImColor(255, 255, 255, 145), g.IO.DeltaTime * 12.f);

    const ImU32 col = GetColorU32((held && hovered) ? ImGuiCol_ButtonActive : hovered ? ImGuiCol_ButtonHovered : ImGuiCol_Button);
    RenderNavHighlight(bb, id);

    //GetWindowDrawList()->AddRectFilledMultiColor(bb.Min, bb.Max, GetColorU32(it_anim->second.background), GetColorU32(it_anim->second.background1), GetColorU32(it_anim->second.background1), GetColorU32(it_anim->second.background));

    GetWindowDrawList()->AddImage(image, bb.Min + ImVec2(0, 3), bb.Min + ImVec2(21, 24), ImVec2(0, 0), ImVec2(1, 1), GetColorU32(it_anim->second.background));

    PushStyleColor(ImGuiCol_Text, GetColorU32(it_anim->second.text));
    RenderTextClipped(bb.Min + ImVec2(28,0) + style.FramePadding, bb.Max - style.FramePadding, label, NULL, &label_size, style.ButtonTextAlign, &bb);
    PopStyleColor();
...
extern ImFont* Alexandria_Regular_2;
bool ImGui::Info_Button(const char* label, ImTextureID image, const ImVec2& size_arg, ImGuiButtonFlags flags)
{
    ImGuiWindow* window = GetCurrentWindow();
    if (window->SkipItems)
        return false;
   ...
   // bool pressed = ButtonBehavior(bb, id, &hovered, &held, flags);
    ...
    const ImU32 col = GetColorU32((held && hovered) ? ImGuiCol_ButtonActive : hovered ? ImGuiCol_ButtonHovered : ImGuiCol_Button);
    RenderNavHighlight(bb, id);
    GetWindowDrawList()->AddRectFilled(bb.Min, bb.Max, GetColorU32(c::bg_info), 10.f);
    //GetWindowDrawList()->AddRectFilledMultiColor(bb.Min, bb.Max, GetColorU32(it_anim->second.background), GetColorU32(it_anim->second.background1), GetColorU32(it_anim->second.background1), GetColorU32(it_anim->second.background));

    GetWindowDrawList()->AddImage(image, bb.Min + ImVec2(15,14), bb.Min + ImVec2(40, 40), ImVec2(0, 0), ImVec2(1, 1), GetColorU32(c::text_active));

    PushFont(Alexandria_Regular_2);

    GetWindowDrawList()->AddText(ImVec2(bb.Min.x + 57, bb.Min.y + 13), GetColorU32(c::text), label);
...
bool ImGui::Warning_Button(const char* label, ImTextureID image, const ImVec2& size_arg, ImGuiButtonFlags flags)
{
    ImGuiWindow* window = GetCurrentWindow();
    if (window->SkipItems)
        return false;
    ...
    GetWindowDrawList()->AddImage(image, bb.Min + ImVec2(0, 2), bb.Min + ImVec2(30, 32), ImVec2(0, 0), ImVec2(1, 1), GetColorU32(c::text_active));

    PushFont(Alexandria_Regular_2);

    GetWindowDrawList()->AddText(ImVec2(bb.Min.x + 42, bb.Min.y + 1), GetColorU32(c::text_warning), label);
...
bool ImGui::Succes_Button(const char* label, ImTextureID image, const ImVec2& size_arg, ImGuiButtonFlags flags)
{
    ImGuiWindow* window = GetCurrentWindow();
    if (window->SkipItems)
        return false;
    ...
    GetWindowDrawList()->AddImage(image, bb.Min + ImVec2(0, 0), bb.Min + ImVec2(34, 34), ImVec2(0, 0), ImVec2(1, 1), GetColorU32(c::text_active));

    PushFont(Alexandria_Medium);

    GetWindowDrawList()->AddText(ImVec2(bb.Min.x + 45, bb.Min.y + 7), GetColorU32(c::text_active), label);
...
bool ImGui::Enjoy_Button(const char* label, ImTextureID image, const ImVec2& size_arg, ImGuiButtonFlags flags)
{
    ImGuiWindow* window = GetCurrentWindow();
    if (window->SkipItems)
        return false;
...
bool ImGui::Exit_Button(const char* label, ImTextureID image, const ImVec2& size_arg, ImGuiButtonFlags flags)
{
    ImGuiWindow* window = GetCurrentWindow();
    if (window->SkipItems)
        return false;
    ...
    it_anim->second.background = ImLerp(it_anim->second.background, IsItemActive() || hovered ? c::main_color : c::con_text_active, g.IO.DeltaTime * 12.f);
    ...
    GetWindowDrawList()->AddImage(image, bb.Min, bb.Max, ImVec2(0, 0), ImVec2(1, 1), GetColorU32(it_anim->second.background));
...
bool ImGui::Circle_Button(const char* label, ImTextureID image, const ImVec2& size_arg, ImGuiButtonFlags flags)
{
    ImGuiWindow* window = GetCurrentWindow();
    if (window->SkipItems)
        return false;
    ...
    it_anim->second.background = ImLerp(it_anim->second.background, IsItemActive() || hovered ? c::bg_color_cirlce_icon : c::bg_color_cirlce_icon, g.IO.DeltaTime * 12.f);
    ...
    GetWindowDrawList()->AddRectFilled(bb.Min, bb.Max, GetColorU32(it_anim->second.background),100.f);

    GetWindowDrawList()->AddImage(image, bb.Min + ImVec2(7, 9), bb.Min + ImVec2(29, 27), ImVec2(0, 0), ImVec2(1, 1), GetColorU32(it_anim->second.text));
...
bool ImGui::Checkbox(const char* label, bool* v)
{
    ...
    const ImGuiID id = window->GetID(label);
    const ImVec2 label_size = CalcTextSize(label, NULL, true);
    const float square_sz = 24.f, w = (GetContentRegionMax().x - 203 - g.Style.WindowPadding.x) - square_sz;
    ...
    it_anim->second.background = ImLerp(it_anim->second.background, *v ? c::bg_checkbox_active : c::bg_checkbox_in, g.IO.DeltaTime * 10.f);
    it_anim->second.circle = ImLerp(it_anim->second.circle, *v ? c::cyrcle_color_active : c::cyrcle_color_in_active, g.IO.DeltaTime * 10.f);

    it_anim->second.text = ImLerp(it_anim->second.text, *v || hovered ? c::text_checkbox_active : c::text_checkbox_in_active, g.IO.DeltaTime * 10.f);
    ...
    GetWindowDrawList()->AddText(ImVec2(check_bb.Max.x - square_sz, check_bb.Min.y), GetColorU32(it_anim->second.text), label);
...
bool ImGui::BeginCombo(const char* label, const char* preview_value, float val, bool multi, ImGuiComboFlags flags)
{
    ...
    const ImRect bb(window->DC.CursorPos + ImVec2(11, 27), window->DC.CursorPos + ImVec2(w - 190, 67));
    ...
    GetWindowDrawList()->AddRect(ImVec2(bb.Min.x, bb.Min.y), ImVec2(bb.Max.x, bb.Max.y), GetColorU32(c::rect_combo), 4.f);
    GetWindowDrawList()->AddRectFilled(ImVec2(bb.Min.x + 1, bb.Min.y + 1), ImVec2(bb.Max.x - 1, bb.Max.y - 1), GetColorU32(c::bg_combo), 4.f);
   ...
   // window->DrawList->AddText(ImVec2(bb.Min.x + 5, bb.Min.y - 29), GetColorU32(c::text_active), label);
    ...
    PushStyleColor(ImGuiCol_WindowBg, c::bg_combo);
...
// Getter for the olginCombo/EndCombo block. The preview is designed to only host non-interactive elements
...
bool ImGui::InputTextEx(const char* label, ImTextureID image, const char* hint, char* buf, int buf_size, const ImVec2& size_arg, ImGuiInputTextFlags flags, ImGuiInputTextCallback callback, void* callback_user_data)
{
    ...
    const ImGuiID id = window->GetID(label);
    const ImVec2 label_size = CalcTextSize(label, NULL, true);
    const ImVec2 frame_size = CalcItemSize(size_arg, CalcItemWidth() + 22, (is_multiline ? g.FontSize * 8.0f : label_size.y + 5) + style.FramePadding.y * 4.0f); // Arbitrary default of 8 lines high for multi-line
    const ImVec2 total_size = ImVec2(frame_size.x + (label_size.x > 0.0f ? style.ItemInnerSpacing.x + label_size.x : 0.0f), frame_size.y + 8);

    const ImRect frame_bb(window->DC.CursorPos + ImVec2(0, 6), window->DC.CursorPos + frame_size + ImVec2(0, 40));
    ...
    it_anim->second.rect = ImLerp(it_anim->second.rect, IsItemActive() || hovered ? c::rect_input_active : c::rect_input, g.IO.DeltaTime * 10.f);

    it_anim->second.shadow = ImLerp(it_anim->second.shadow, IsItemActive() || hovered ? c::rect_input_active : c::rect_input_in, g.IO.DeltaTime * 10.f);

    it_anim->second.text_color = ImLerp(it_anim->second.text_color, IsItemActive() || hovered ? c::text_active : c::text_in_active, g.IO.DeltaTime * 10.f);
    ...
    if (!is_multiline)
    {
        ...
        ImGui::GetWindowDrawList()->AddShadowRect(ImVec2(frame_bb.Min.x, frame_bb.Min.y), ImVec2(frame_bb.Max.x, frame_bb.Max.y), GetColorU32(it_anim->second.shadow),60.f,ImVec2(0,0), ImDrawFlags_ShadowCutOutShapeBackground,25.f);
        ImGui::GetForegroundDrawList()->AddRect(ImVec2(frame_bb.Min.x, frame_bb.Min.y), ImVec2(frame_bb.Max.x, frame_bb.Max.y), GetColorU32(it_anim->second.rect), 17.f);
        ImGui::GetForegroundDrawList()->AddRectFilled(ImVec2(frame_bb.Min.x + 74, frame_bb.Min.y + 21), ImVec2(frame_bb.Min.x + 75, frame_bb.Min.y + 48), GetColorU32(c::rect_input), 17.f);

        window->DrawList->AddImage(image, frame_bb.Min + ImVec2(27, 23), frame_bb.Min + ImVec2(51, 46), ImVec2(0, 0), ImVec2(1, 1), GetColorU32(c::text_active));
    ...
    if (render_cursor || render_selection)
    {
            ...
            for (const ImWchar* p = text_selected_begin; p < text_selected_end; )
            {
                ...
                else
                {
                    ...
                    if (rect.Overlaps(clip_rect))
                        draw_window->DrawList->AddRectFilled(rect.Min + ImVec2(101, 20), rect.Max + ImVec2(115, 25), ImColor(30, 30, 30, 200));
                }
        ...
        if (is_multiline || (buf_display_end - buf_display) < buf_display_max_length)
        {
            ...
            draw_window->DrawList->AddText(g.Font, g.FontSize, draw_pos - draw_scroll + ImVec2(101, 22), col, buf_display, buf_display_end, 0.0f, NULL);
        ...
        if (render_cursor)
        {
            ...
            if (cursor_is_visible && cursor_screen_rect.Overlaps(clip_rect)) {
                ...
                draw_window->DrawList->AddRectFilled(frame_bb.Min + ImVec2(it_anim->second.value + 100, 24), frame_bb.Min + ImVec2(it_anim->second.value + 101, 45), ImColor(255, 255, 255));
            }
    ...
    else
    {
        ...
        if (is_multiline || (buf_display_end - buf_display) < buf_display_max_length)
        {
            ...
            ImU32 col = GetColorU32(it_anim->second.text_color);
            draw_window->DrawList->AddText(g.Font, g.FontSize, draw_pos + ImVec2(101, 22), col, buf_display, buf_display_end, 0.0f, NULL);
...
bool ImGui::Selectable(const char* label, bool selected, ImGuiSelectableFlags flags, const ImVec2& size_arg)
{
  ...
  //  it_anim->second.circle = ImLerp(it_anim->second.circle, selected ? c::circle_active : c::circle_inactive, g.IO.DeltaTime * 14.f);
    ...
    it_anim->second.cirlce_active = ImLerp(it_anim->second.cirlce_active, selected ? c::main_color : ImColor(0,0,0,0), g.IO.DeltaTime * 14.f);
  ...
  //  GetWindowDrawList()->AddCircleFilled(bb.Max - ImVec2(23 + it_anim->second.move, 15), 8.f, GetColorU32(it_anim->second.circle));
    ...
    GetWindowDrawList()->AddRectFilled(bb.Min + ImVec2(0 + it_anim->second.move, 15), bb.Min + ImVec2(3 + it_anim->second.move, 25), GetColorU32(it_anim->second.cirlce_active),4.f);

    GetWindowDrawList()->AddText(ImVec2(bb.Min.x + 13 + it_anim->second.move, bb.Min.y + 10), GetColorU32(it_anim->second.text), label);
